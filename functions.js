...
export const getConnectionData = (req) => {
  if (req && req.session && req.session.grant && req.session.grant.step1 && req.session.grant.response) {
    ...
    const connectionData = { user_nsid, username, fullname, oauth_token, oauth_token_secret, access_token, access_secret };
...
const showConnectionData = (connectionData) => {
  const { user_nsid, username, fullname, oauth_token, oauth_token_secret, access_token, access_secret } = connectionData;
